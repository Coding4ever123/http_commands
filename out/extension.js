...
exports.deactivate = exports.activate = exports.send = exports.historypath = exports.inputpath = void 0;
...
const linux = path_1.join(func.getEnv("HOME"), ".config");
const dirname = path_1.join({
    win32: func.getEnv("APPDATA"),
    linux,
    darwin: path_1.join(func.getEnv("HOME"), "Library", "Application Support"),
    freebsd: linux,
    openbsd: linux,
    aix: linux,
}[process.platform], "AdrianDoesCoding-http");
const savespath = path_1.join(dirname, "saves");
exports.inputpath = path_1.join(dirname, "input");
exports.historypath = path_1.join(dirname, "history");
const headerpicks = (arr) => {
    return func.sortascending(Object.entries(arr).map((x) => `${x[0]}: ${x[1]}`));
};
async function send(req, ip, res) {
    if (!res) {
        if (!req.method && !req.url) {
            vscode.window.showWarningMessage(`Missing Method and URL of the request`);
        }
        else if (!req.method) {
    ...
    if (!res) {
        ...
        let path = path_1.join(exports.historypath, `${date} ${encodeURIComponent(req.url)} 0`);
        let a = { ...req, ip };
    ...
    while (isfinished) {
        ...
        switch (input) {
            ...
            case responsenamelist["Body"]:
                ...
                let inbody = true;
                while (inbody) {
                    let resp = await vscode.window.showQuickPick(["Show body", "Save body"], func.quickpickoptions);
                    if (!resp)
                        inbody = false;
                    else {
                        body = await func.formatcode(e.body, e.headers["content-type"].split(";")[0]);
                    }
                    if (resp === "Show body")
                        await vscode.window.showQuickPick(body.split("\n"), func.quickpickoptions);
                    else if (resp === "Save body") {
                        let formatted = await vscode.window.showQuickPick(["formatted", "unformatted"], func.quickpickoptions);
                        if (formatted) {
            ...
            case responsenamelist["Timings"]:
                await func.quickpick(Object.entries(e.timings).map(([a, b]) => `${a}: ${func.convertBest("ns", b)}`));
                break;
            case responsenamelist["Save"]:
                ...
                var readpath = path_1.join(savespath, savename);
                let a;
                if (fs.existsSync(readpath)) {
                    a = await vscode.window.showWarningMessage("Save already exists, do you want to overwrite", "Yes", "No");
                }
                if (a !== "Yes")
...
async function activate(context) {
    func.makedirs(dirname, savespath, exports.historypath, exports.inputpath);
    context.subscriptions.push(vscode.commands.registerCommand("Adrian.http.cleartemp", func.cleartemp), vscode.commands.registerCommand("Adrian.http.clearhistory", h.clear), vscode.commands.registerCommand("Adrian.http.history", h.history), vscode.commands.registerCommand("Adrian.http.fund", func.fund), vscode.commands.registerCommand("Adrian.http.speed", speedtest_1.speed), vscode.commands.registerCommand("Adrian.http.request", async () => {
            ...
            let namelist = {
                ...
                Headers: "Headers" +
                    (Object.keys(req.headers).length != 0
                        ? `: ${func.get_HeaderString(req.headers, 0, false)}`
                        : ""),
                Params: "Params" +
                    ...
                    while (dourl) {
                        ...
                        try {
                            ...
                            if (url.protocol == "http:" ||
                                url.protocol == "https:") {
                                req.url = `${url.hostname}${func.removetrailingslash(url.pathname)}`;
                                if (url.port ||
                                    Object.keys(url.searchParams).length !==
                                        0 ||
                                    url.hash.length !== 0)
                                    vscode.window.showInformationMessage("port, hash and search params did not take affect.\nplease try the dedicated categorys.");
                                dourl = false;
                                urlinp = undefined;
                                ip = await func.getAddress(url.hostname);
                                if (ip instanceof Error) {
                                    vscode.window.showErrorMessage("Failed to resolve URL: " +
                                        ip.message);
                                }
                                break;
                            }
                            else {
                                vscode.window.showWarningMessage("Invalid protocol");
                            }
                        }
                        catch {
                ...
                case namelist["Body"]:
                    ...
                    while (inbody) {
                        ...
                        if (!inp)
                            ...
                            let writepath = req.body.length != 0
                                ? req.body
                                : path_1.join(exports.inputpath, func.getFileName(req["content-type"]));
                            ...
                            let a = await func.quickpick(Object.entries(func.FILEextensions).map(([a, b]) => `${a}: ${b.type}`));
                            if (a)
                                req["content-type"] = a.split(": ")[0];
                        }
                    }
                    break;
                case namelist["Hash"]:
                ...
                case "Saves":
                    ...
                    while (savesopen) {
                        ...
                        if (!inp)
                            ...
                            let data = JSON.parse(fs.readFileSync(readpath).toString());
                            ...
                            ip = await func.getAddress(req.url.split("/")[0]);
                            savesopen = false;
                        }
                        else if (inp === "delete") {
