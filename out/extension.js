...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("Adrian.http.request", async () => {
        ...
        let [isfinished, dourl, notsent, noheaderselected] = new Array(4).fill(true);
        ...
        while (notsent) {
            ...
            let namelist = {
                ...
                Hash: "Hash" + (req.hash.length != 0 ? `: ${req.hash}` : ""),
                Headers: "Headers" +
                        ...
                        while (isfinished) {
                            let responsenamelist = {
                                ...
                                Headers: "Headers" +
                                    (req.headers.length != 0
                                        ...
                                        : ""),
                                Timings: `Timings: ${func.convert_seconds(e.timings.total)}`,
                                Body: "Body" +
                                    (req.body.length != 0
                                        ? `: ${e.body}`
                                        : ""),
                            ...
                            switch (input) {
                                case responsenamelist["Headers"]:
                                    await vscode.window.showQuickPick(headerpicks(e.headers));
                                    break;
                                case responsenamelist["Body"]:
                                    let body = await func.formatcode(e.body, e.headers["content-type"].split(";")[0]);
                                    await vscode.window.showQuickPick(body.split("\n"));
                                    break;
                                case responsenamelist["Timings"]:
                                    await vscode.window.showQuickPick(Object.entries(e.timings).map(([a, b]) => `${a}: ${func.convert_seconds(b)}`));
                                    break;
